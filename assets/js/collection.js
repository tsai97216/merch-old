...
export function createCollection({ collection, items, works, detail }) {
  ...
  if (search) {
    search.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i><input type="search" placeholder="搜尋標題、作品、角色、備註…" aria-label="搜尋收藏">';
  }
  ...
  if (filters) {
    filters.innerHTML = filterDefinitions.map(([name, label, options]) => `<label class="select-placeholder"><span class="sr-only">${label}</span><select data-filter="${name}"><option value="">${label}</option>${options.map(([value, option]) => `<option value="${value}">${option}</option>`).join('')}</select></label>`).join('');
  }
  ...
  function filteredItems() {
    ...
    const result = items.filter((item) => {
      ...
      return (!query || haystack.includes(query)) &&
        (!state.work || item.workId === state.work) &&
        (!state.character || (item.characters || []).includes(state.character)) &&
  ...
  function card(item) {
    ...
    return `<article class="item-card" data-item-id="${item.id}" tabindex="0" role="button"><div class="item-image">IMAGE</div><h4>${item.title}</h4><p>${item.workName}${characters ? ` · ${characters}` : ''}</p><div class="item-bottom"><span class="tag ${item.status === 'received' ? 'received' : 'pending'}">${statusLabel(item.status)}</span><b>${money(priceOf(item))}</b></div></article>`;
  }

  function row(item) {
    const characters = (item.characters || []).join('、');
    return `<article class="collection-list-row" data-item-id="${item.id}" tabindex="0" role="button"><div class="list-image">IMAGE</div><div class="list-main"><h4>${item.title}</h4><p>${item.workName}${characters ? ` · ${characters}` : ''}</p></div><span class="tag ${item.status === 'received' ? 'received' : 'pending'}">${statusLabel(item.status)}</span><span class="list-type">${item.category || '其他'}</span><strong>${money(priceOf(item))}</strong></article>`;
  }
