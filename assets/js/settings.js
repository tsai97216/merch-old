import { github } from './github.js?v=2.12.3';
import { getVersion } from './version.js?v=2.12.3';
...
export function createSettings({ settings }) {
  ...
  connect.onclick = async () => {
    ...
    message.textContent = '正在驗證 Admin Secret 與 GitHub 連線…';
    ...
    try {
      ...
      github.disconnect();
      message.textContent = error.message === '已取消。' ? '已取消。' : `連線失敗：${error.message}`;
      message.dataset.kind = error.message === '已取消。' ? '' : 'error';
      render();
