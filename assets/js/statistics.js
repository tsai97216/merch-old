...
export function createStatistics({ statistics, works, items }) {
  if (!statistics) return { render() {} };
  const overview = statistics.querySelector('.stats-overview');
  const chartArea = statistics.querySelector('.two-column');
  if (!overview || !chartArea) return { render() {} };
  let activeWork = 'all';
  const tabs = document.createElement('div');
  tabs.className = 'stats-work-tabs';
  tabs.innerHTML = `<button class="stats-work-tab is-active" data-work="all">全部</button>${works.map((work) => `<button class="stats-work-tab" data-work="${work.id}">${work.name}</button>`).join('')}`;
  ...
  function render() {
    ...
    overview.innerHTML = [
      ['COLLECTION', summary.total, '收藏件數'], ['RECEIVED', summary.received, '已收到'], ['PENDING', summary.pending, '尚未收到'],
      ['TOTAL SPENDING', money(summary.totalSpending), '全部收藏'], ['THIS MONTH', money(summary.thisMonth), '本月花費'], ['THIS YEAR', money(summary.thisYear), '今年花費']
