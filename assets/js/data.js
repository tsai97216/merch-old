...
async function fetchSnapshot() {
  ...
  const works = await Promise.all(index.data.works.map(async (work) => {
    ...
    return { ...work, ...data, id: work.id, name: work.name, items: Array.isArray(data.items) ? data.items.map((item) => ({ ...item, workId: work.id, workName: work.name })) : [] };
  }));
...
async function reload() {
  ...
  reloadPromise = fetchSnapshot().then((snapshot) => {
...
function createStore(snapshot) {
  return {
    ...
    addItem(item) {
      ...
      work.items.push(normalized); this.items.push(normalized); notify(); return normalized;
    },
    updateItem(id, patch) {
    ...
    removeItem(id) {
      ...
      if (work) { const workIndex = work.items.indexOf(removed); if (workIndex >= 0) work.items.splice(workIndex, 1); }
      notify(); return true;
    },
...
function notify() {
  listeners.forEach((listener) => { try { listener(cache); } catch (error) { console.error('[Chi MERCH] render listener error', error); } });
}

window.addEventListener('chi-merch:github', () => { if (cache) reload().catch((error) => console.error('[Chi MERCH] data reload error', error)); });

export async function loadData() { return reload(); }
export async function reloadData() { return reload(); }
export function clearDataCache() { cache = null; reloadPromise = null; }
